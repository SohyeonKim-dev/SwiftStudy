...
/*
 ...
 익스텐션(Extension) 은 스위프트의 강력한 기능 중 하나입니다. -> 신기방기함
 익스텐션은 구조체, 클래스, 열거형, 프로토콜 타입에 새로운 __기능을 추가__할 수 있는 기능입니다.
...
* 클래스의 상속과 익스텐션을 비교해보겠습니다. 이 둘은 비슷해보이지만 실제 성격은 많이 다릅니다.
 ...
 또 클래스의 상속은 특정 타입을 물려받아 하나의 새로운 타입을 정의하고 추가 기능을 구현하는 수직 확장이지만,
 익스텐션은 기존의 타입에 기능을 추가하는 수평 확장입니다. (오,,!)
 또, 상속을 받으면 기존 기능을 재정의할 수 있지만, 익스텐션은 재정의할 수 없다는 것도 큰 차이 중 하나입니다. : 오버라이딩
 상황과 용도에 맞게 상속과 익스텐션을 선택하여 사용하면 됩니다.

 
 익스텐션을 사용하는 대신 원래 타입을 정의한 소스에 기능을 추가하는 방법도 있겠지만,
 외부 라이브러리나 프레임워크를 가져다 썼다면 원본 소스를 수정하지 못합니다.
 이처럼 외부에서 가져온 타입에 내가 원하는 기능을 추가하고자 할 때 익스텐션을 사용합니다. (객체지향에서 중요한 관점일 듯)
 
 따로 상속을 받지 않아도 되며, 구조체와 열거형에도 기능을 추가할 수 있으므로 익스텐션은 매우 편리한 기능입니다.
 익스텐션은 모든 타입에 적용할 수 있습니다. : 구조체, 열거형, 클래스, 프로토콜, 제네릭 타입 등을 뜻합니다.
 즉, 익스텐션을 통해 모든 타입에 연산 프로퍼티, 메서드, 이니셜라이저, 서브스크립트, 중첩 데이터 타입 등을 추가할 수 있습니다.
 
 더불어 익스텐션은 __프로토콜과 함께 사용__하면 굉장히 강력한 기능을 선사합니다.
 이 부분은 __프로토콜 중심 프로그래밍(Protocol Oriented Programming)__에 대해 더 알아보면 좋습니다.
 : POP -> 이런 개념도 있구만..! 프로토콜 위주로 프로그래밍..?
 ...
 실제로 Double 타입의 정의를 살펴보면 그 모든것이 다 정의되어 있지는 않습니다.
 
 그러면 Double 타입이 채택하고 준수해야 하는 수많은 프로토콜은 어디로 갔을까요?
...
// 진짜 좀 신기방기 하긴 함. 저게 우리가 쓰는 int 그 자체라니..

/*
 
 위 코드의 익스텐션은 Int 타입에 두 개의 연산 프로퍼티를 추가한 것입니다.
